...
from PySide2.QtCore import Qt, QTimer, QUrl, QSize
from PySide2.QtGui import QCursor, QDesktopServices, QIcon
...
class DownloadView(QtWidgets.QWidget, Ui_Download, DownloadStatus):
    ...
    def GetDownloadWaifu2xFilePath(self, bookId, index):
        if bookId not in self.downloadDict:
            return ""
        task = self.downloadDict[bookId]
        return os.path.join(task.convertPath, "{:04}".format(index + 1))

    def GetDownloadFilePath(self, bookId, index):
        if bookId not in self.downloadDict:
            return ""
        task = self.downloadDict[bookId]
        return os.path.join(task.savePath, "{:04}".format(index + 1))
