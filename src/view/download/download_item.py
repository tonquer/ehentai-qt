...
class DownloadItem(QtTaskBase):
    ...
    def DownloadInit(self):
        ...
        if 0 < self.maxDownloadPic <= self.curDownloadPic:
            return self.Success

        return self.Downloading

    # 初始化成功回调
    def DownloadInitCallBack(self, bookName, maxPic):
        if not self.title:
            self.title = bookName
            self.dirty = True
        self.maxDownloadPic = maxPic
    ...
    def GetDownloadPath(self):
        ...
        if self.maxDownloadPic <= 0 or not self.title:
            return 0, "", True
        ...
        if not self.savePath and Setting.SavePath.value:
            ...
            if Setting.SaveNameType.value == SaveNameType.TitleType:
                ...
                self.savePath = os.path.join(path2, "default")
            elif Setting.SaveNameType.value == SaveNameType.TypeTitle:
                path2 = os.path.join(path, "default")
                self.savePath = os.path.join(path2, ToolUtil.GetCanSaveName(self.title))
        ...
        savePath = os.path.join(self.savePath, "{:04}".format(self.curDownloadPic + 1))
        return self.curDownloadPic, savePath, False
    ...
    def ConvertInit(self):
        ...
        #     else:
        #         break
        if self.curPreConvertId >= self.maxDownloadPic:
            return self.ConvertSuccess

        if self.curPreConvertId >= self.curDownloadPic:
            return self.Waiting
