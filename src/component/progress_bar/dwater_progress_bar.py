...
class DWaterProgress(QProgressBar):
    ...
    def resizePixmap(self, sz):
        ...
        if self.waterFrontImage.size() != waterSize:
            image = QImage(waterWidth, waterHeight, QImage.Format.Format_ARGB32)
        ...
        if self.waterBackImage.size() != waterSize:
            image = QImage(waterWidth, waterHeight, QImage.Format.Format_ARGB32)
            image.fill(Qt.GlobalColor.transparent)  # partly transparent red-ish background
            waterPainter = QPainter(image)
    ...
    def paint(self, painter):
        ...
        waterPainter.setRenderHint(QPainter.RenderHint.Antialiasing)
        ...
        if self.isTextVisible():
            ...
            if progressText == '100':
                ...
                waterPainter.setPen(Qt.GlobalColor.white)
                waterPainter.drawText(rectValue, Qt.AlignmentFlag.AlignCenter, progressText)
            else:
                ...
                waterPainter.drawText(rectPerent, Qt.AlignmentFlag.AlignCenter, '%')

        waterPainter.end()

        maskPixmap = QPixmap(sz)
        maskPixmap.fill(Qt.GlobalColor.transparent)
        path = QPainterPath()
        path.addEllipse(QRectF(0, 0, sz.width(), sz.height()))
        maskPainter = QPainter()
        maskPainter.begin(maskPixmap)
        maskPainter.setRenderHint(QPainter.RenderHint.Antialiasing)
        maskPainter.setPen(QPen(Qt.GlobalColor.white, 1))
        maskPainter.fillPath(path, QBrush(Qt.GlobalColor.white))
        maskPainter.end()
        ...
        contentPainter.fillRect(contentImage.rect(), Qt.GlobalColor.transparent)
