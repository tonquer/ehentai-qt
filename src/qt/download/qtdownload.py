...
class QtDownload(QtWidgets.QWidget, Ui_download):
    ...
    def GetDonwloadFilePath(self, bookId, index):
        if bookId not in self.downloadDict:
            return ""
        task = self.downloadDict[bookId]
        return os.path.join(task.savePath, "{:04}.{}".format(index + 1, "jpg"))
    ...
    def HandlerDownloadList(self):
        ...
        for _ in range(addNum):
            ...
            for task in list(self.downloadList):
                ...
                task.status = DownloadInfo.DownloadCover
    ...
    def GetStatusText(self, status):
        ...
        elif status == DownloadInfo.DownloadCover:
            status = self.tr("正在下载封面")
    ...
    def UpdateTableItem(self, info):
        ...
        self.tableWidget.setItem(info.tableRow, 3, QTableWidgetItem(self.GetStatusText(info.status)))
    ...
    def SelectMenu(self, pos):
        ...
        if index.isValid():
            ...
            if len(selectRows) == 1:
                ...
                elif task.status in [DownloadInfo.DownloadCover, DownloadInfo.Downloading, DownloadInfo.Waiting,
    ...
    def OpenBookInfo(self):
        ...
        QtOwner().owner.bookInfoForm.OpenBook(bookId, task.token, task.domain)
