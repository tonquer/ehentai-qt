...
class ItemWidget(QWidget):
    def __init__(self, _id, title, index, info, param, url, *args, **kwargs):
...
class QtBookList(QListWidget):
    ...
    def AddBookItem(self, _id, title, info="", url="", path="", param="", originalName=""):
        index = self.count()
        iwidget = ItemWidget(_id, title, str(index+1), info, param, url, self)
    ...
    def ReDownloadPicture(self):
        ...
        for item in selected:
            ...
            if widget.url and config.IsLoadingPicture:
                ...
                QtTask().AddDownloadTask(widget.url, "", None, self.LoadingPictureComplete, True, index, False,
                                         self.GetName())
                pass
