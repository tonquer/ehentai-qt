...
class CompleteLineEdit(QLineEdit):
    ...
    def keyPressEvent(self, ev):
        if not self.listView.isHidden():
            ...
            if Qt.Key_Down == key:
                curText = re.split('&|\|', self.text().replace("\"", "").strip("$"))
                curText = curText.pop() if len(curText) > 0 else ""
                text = currentIndex.data() if not currentIndex.data() else currentIndex.data().split("|")[0]
                if currentIndex.row() <= 0 and curText != text:
                    row = 0
                else:
